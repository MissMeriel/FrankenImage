...
class DatasetGenerator(keras.utils.Sequence):
    ...
    def process_training_dir(self, trainingdir, imgsize=(135, 240)):
        td = os.listdir(trainingdir)
        td.remove("data.csv")
        datasetsize = len(td)
        X_train = np.empty((datasetsize, *imgsize, 3))
        steering_Y_train = np.empty((datasetsize))
        throttle_Y_train = np.empty((datasetsize))
        hashmap = self.process_csv("{}/data.csv".format(trainingdir))
        for index, img in enumerate(td):
            img_file = "{}/{}".format(trainingdir, img)
            image = Image.open(img_file)
            image = image.resize(imgsize[::-1])
            X_train[index] = np.asarray(image)
            steering_Y_train[index] = float(hashmap[img][0])
            throttle_Y_train[index] =  float(hashmap[img][4])
